...
describe('querySpec', function () {
    ...
    it("should return the right result if given a valid query", function () {
        return insightFacade.addDataset('courses', data).then(function (response) {
            ...
            return insightFacade.performQuery(query).then(function (result) {
                chai_1.expect(result).to.deep.equal(ans);
            })
                .catch(function (err) {
                chai_1.expect(err).to.deep.equal(ans);
            });
        });
    });
    ...
    it("test COLUMNS containing only APPLY key", function () {
        ...
        var ans = { 'code': 200, 'body': { "render": "TABLE", "result": [{ "sum": 2655 }, { "sum": 2698 }, { "sum": 2994 }] } };
        return insightFacade.performQuery(query).then(function (result) {
            chai_1.expect(result).to.deep.equal(ans);
        })
            .catch(function (err) {
    ...
    it("test empty APPLY", function () {
        ...
        var ans = { 'code': 200, 'body': { "render": "TABLE", "result": [
                    { "courses_title": "strat univ writ" },
                    { "courses_title": "strat univ writ" },
                    { "courses_title": "strat univ writ" }
                ]
